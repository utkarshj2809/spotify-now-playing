...
import { initiateSpotifyLogin, getClientId, BUILT_IN_CLIENT_ID } from '../utils/spotify';
import './Setup.css';

const REDIRECT_HINT = window.location.origin + window.location.pathname;

// ── Simple connect screen (shown when the app owner has baked in their Client ID) ──
function ConnectScreen() {
  ...
  async function handleConnect() {
    ...
    try {
      ...
      setError(err.message || 'Could not start Spotify login. Please try again.');
      setBusy(false);
    }
  }
  ...
  return (
    <div className="setup-root">
      ...
      <div className="setup-card">
        ...
        <button className="setup-btn" onClick={handleConnect} disabled={busy}>
          {busy ? (
            <>
...
// ── Manual setup form (shown when no built-in Client ID is configured) ──
...
export default function Setup() {
  return BUILT_IN_CLIENT_ID ? <ConnectScreen /> : <ManualSetup />;
}
